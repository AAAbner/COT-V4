...
void master_broadcast_data()
{
	...
	s_cot_sample_msg_tx->data[2] = 0x01;					//多节点对多锚点新增的字节    0x01代表是节点的广播帧
	uint8_t temp[64];
	memset(temp,0x5a,64);
...
void slave_broadcast_data(uint32_t device_id)
{
	...
	s_cot_sample_msg_tx->data[0] = 0x00;
	s_cot_sample_msg_tx->data[1] = 0x02;
	s_cot_sample_msg_tx->data[2] = 0x00;			//多节点对多锚点新增的字节    0x00代表是锚点回复的广播帧
	uint8_t temp[64];
	memset(temp,0x5a,64);
...
static void cot_sample_msg_handle( cot_mac_msg_t *sample_msg_packet )					//消息处理函数	
{
	...
	sample_msg->data = &sample_msg_packet->data[2];														//sample_msg_packet->data域的第三个字节开始赋值到
	uint8_t cmd_h;
					...
					case SAMPLE_APP_CMD_BROADCAST:
						{
								...
								if(sample_msg->data[0] != 0x01)//多移动点广播标志位		，当收到的广播帧不是其他节点的广播帧
								{
...
uint16_t sample_app_processevent(uint8_t task_id, uint16_t events)
{
    ...
    if(events & SMAPLE_MSG_TX_EVENT) // 发送数据
    {
        ...
        if( p_msg )
        {
			...
			p_msg->rssi_water_leve = -80;						//rssi检测阈值为-80
            p_msg->data = (uint8_t *)(p_msg+1);
	...
	{
		/*设置mac层空闲状态*/
		cot_mac_msg_t *p_msg = (cot_mac_msg_t*)osal_msg_allocate( sizeof(cot_mac_msg_t) );				//？？？？？？？？？？新增
        if( p_msg )
        {
            p_msg->hdr.event = CMD_MAC_LAYER;
    ...
    if ( events & SMAPLE_MSG_RX_ERROR_EVENT )						//消息接收错误事件，同上
    {
